...
// Displays pulsing alert, contact notification list, and WhatsApp link
                ...
                {contacts.slice(0, 3).map((c, i) => (
                  <div key={i} className="flex items-center justify-between">
                    ...
                    <a href={c.whatsapp_link} target="_blank" rel="noreferrer"
                      className="text-xs text-[#25D366] font-bold">Open WA</a>
                  </div>
                ))}
              </div>
            </motion.div>
...
function Step({ done, active, color, icon, label }) {
  return (
