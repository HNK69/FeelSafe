...
export default function FeedbackModal({ tripId, onClose, onSubmitted }) {
  ...
  const routeId = tripId ? `trip_${tripId}` : 'general';
  ...
  const handleSubmit = async () => {
    ...
    setSubmitting(true);
    try {
      await submitRouteFeedback(routeId, rating, isUnsafe, feedback.trim() || PROMPTS[rating]);
      setDone(true);
  ...
  return (
    <AnimatePresence>
      <motion.div
        ...
        style={{ background: 'rgba(11,16,32,0.88)', backdropFilter: 'blur(10px)' }}>
          ...
          <AnimatePresence mode="wait">
            {done ? (
              ...
              <motion.div key="form" initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
                ...
                {/* Unsafe toggle */}
                <button onClick={() => setIsUnsafe(v => !v)}
