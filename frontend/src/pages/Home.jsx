// pages/Home.jsx — real backend stats, no fake counters
    ...
    <div className="min-h-screen px-6 pt-10 pb-24 md:px-20 max-w-7xl mx-auto">
      ...
      <div className="text-center mt-10 md:mt-20">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }}
          ...
          <Link to="/safe-route"
            ...
            <Users className="w-5 h-5 text-[#7C4DFF]" /> Community Intel
          </Link>
        </motion.div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-24">
        <FeatureCard icon={<Bell className="w-8 h-8 text-[#FFC857]" />}
          ...
          desc="Describe your situation. AI classifies risk instantly and auto-alerts emergency contacts." delay={0.4} />
        <FeatureCard icon={<Map className="w-8 h-8 text-[#00E5FF]" />}
          title="SafeRoute AI"
          desc="See safest, fastest, and alternative routes — each scored with AI reasoning and community data." delay={0.5} />
        <FeatureCard icon={<Zap className="w-8 h-8 text-[#7C4DFF]" />}
          title="Auto Escalation"
          desc="MEDIUM or HIGH risk? Contacts are WhatsApp-alerted instantly — no manual step needed." delay={0.6} />
      </div>
      ...
      <motion.div initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }}
        ...
        <div className="glass rounded-3xl p-10 max-w-4xl mx-auto border border-[#00E5FF]/20 relative overflow-hidden">
          ...
          <p className="text-gray-400 text-sm mb-8">
            {stats ? 'Live data from FeelSafe database' : 'Start backend to see live stats'}
          </p>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-8">
            <Stat value={stats?.total_trips ?? '—'}       label="Total Trips"      live={!!stats} />
            <Stat value={stats?.active_trips ?? '—'}      label="Active Now"       live={!!stats} pulse />
            <Stat value={stats?.sos_alerts ?? '—'}        label="SOS Alerts"       live={!!stats} />
            <Stat value={stats?.community_reports ?? '—'} label="Safety Reports"   live={!!stats} />
          </div>
        </div>
      </motion.div>
      ...
      <motion.div initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }}
        ...
        <div className="flex flex-col md:flex-row items-center justify-center gap-4 max-w-3xl mx-auto">
          {[['1','Enter destination','#00E5FF'],['2','AI picks safest route','#7C4DFF'],
            ['3','Trip monitored live','#FFC857'],['4','Rate & help community','#00FF9D']
          ].map(([n, l, c], i) => (
...
function Stat({ value, label, live, pulse }) {
  return (
    <div>
      <div className={`text-4xl font-black text-white mb-2 text-gradient ${pulse ? 'animate-pulse' : ''}`}>{value}</div>
      <div className="text-sm font-medium text-gray-400 uppercase tracking-wider flex items-center justify-center gap-1">
        {live && <span className="w-1.5 h-1.5 rounded-full bg-[#00FF9D] animate-pulse" />}
        {label}
      </div>
    </div>
  );
