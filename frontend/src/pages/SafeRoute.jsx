// pages/SafeRoute.jsx
// Community Safety Hub — route history, live feed, submit feedback
// NOT a duplicate of StartTrip — this is community intelligence + history

import { useState, useEffect } from 'react';
import { motion, AnimatePresence } from 'framer-motion';
import { Users, MapPin, Star, Activity, Clock, Shield, RefreshCw, AlertTriangle } from 'lucide-react';
import { getCommunityFeed, getTripHistory, submitRouteFeedback, getRouteStats } from '../services/api';

const SEV_COLOR = { HIGH: '#FF3B5C', MEDIUM: '#FFC857', LOW: '#00FF9D' };
...
export default function SafeRoute() {
  ...
  const [quickRating, setQuickRating] = useState({});   // routeId → rating pending
  ...
  useEffect(() => {
    load();
    const t = setInterval(() => getCommunityFeed(15).then(r => { if (r?.success) setFeed(r.feed); }), 30000);
    return () => clearInterval(t);
  }, []);

  const handleQuickRate = async (tripId, rating) => {
    const routeId = `trip_${tripId}`;
    await submitRouteFeedback(routeId, rating, false, '');
    ...
    <div className="min-h-screen px-4 md:px-8 py-8 max-w-5xl mx-auto">
      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-3xl font-black flex items-center gap-3">
            <Users className="w-8 h-8 text-[#7C4DFF]" /> Community Safety
          </h1>
      ...
      <div className="flex gap-2 mb-6">
        {[['feed', 'Live Intel Feed', <Activity className="w-4 h-4" />],
          ['history', 'My Trip History', <Clock className="w-4 h-4" />]].map(([id, label, icon]) => (
      ...
      <AnimatePresence mode="wait">
        ...
        {/* ── TAB: Live Intel Feed ─────────────────────────────────────────── */}
        {activeTab === 'feed' && (
          <motion.div key="feed" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
            exit={{ opacity: 0 }}>
            {loading ? (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                {feed.map((ev, i) => (
                  <motion.div key={i}
                    ...
                    className="glass p-4 rounded-2xl border transition-all hover:border-gray-600"
                    style={{ borderColor: `${ev.color}33` }}>
                    <div className="flex items-start gap-3">
                      <div className="w-2.5 h-2.5 rounded-full mt-1.5 flex-shrink-0"
                        style={{ background: ev.color, boxShadow: `0 0 6px ${ev.color}` }} />
                      <div className="flex-1 min-w-0">
                        <div className="font-bold text-sm" style={{ color: ev.color }}>{ev.area}</div>
          ...
          <motion.div key="history" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
            ...
            {loading ? (
              ...
              <div className="glass p-12 rounded-3xl text-center border border-gray-800">
                ...
                <p className="text-gray-600 text-sm mt-1">Complete a trip to see your history here.</p>
              </div>
            ) : (
                      ...
                      <div className="flex items-start justify-between gap-4">
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="text-xs text-gray-500 mt-1 flex items-center gap-2">
                            ...
                            {trip.ended_at ? new Date(trip.ended_at).toLocaleString('en-IN', {
                              day: 'numeric', month: 'short', hour: '2-digit', minute: '2-digit',
                            }) : 'Recently'}
                            {trip.eta_minutes && <span>· ETA was {Math.round(trip.eta_minutes)} min</span>}
                          </div>
                          {trip.feedback_text && (
                            <p className="text-xs text-gray-400 mt-1 italic">"{trip.feedback_text}"</p>
                          )}
                        </div>

                        {/* Rating display or quick-rate */}
                        <div className="flex-shrink-0">
                          {rated ? (
                            <div className="flex items-center gap-1">
                              <Star className="w-4 h-4 fill-[#FFC857] text-[#FFC857]" />
                              <span className="font-black text-lg" style={{ color: rateColor }}>{rated}</span>
                            ...
                            <div>
                              ...
                              <div className="flex gap-1">
                                {[1, 2, 3, 4, 5].map(s => (
                                  <button key={s} onClick={() => handleQuickRate(trip.id, s)}
